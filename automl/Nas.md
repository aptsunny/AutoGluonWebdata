...
|  Highlight  | Dynamic Mini-batch Stochastic Gradient Descent (Dy- namic SGD) that smoothly adjusts the learning rate to stabi- lize the variance changes.        | ☆☆☆☆☆    |
...
comtinious :lr ,wd,dropout,temperate
gradients 
how to efficiently optimize them?
--loss function teaching(learning to teach with dynamic loss function) NeurlPS 2018

find the best loss

discrete: depth width,archri,batchsize
netural architecture Optimization


automl > nas
automl is a pipeline for task

网络结构搜索 超参数搜索
random search 做baseline，

空间小导致 随机搜索也挺好的
大空间验证 好的搜索比 random好的

更多的任务，random search就会有局限，仅仅在cifar10上表现好还不够

搜索算法 没有那么重要， 更重要的是整个pipeline
...
ENAS-RL !!!!
CMU。实效 权重共享
ENAS说明https://www.yanxishe.com/TextTranslation/1600
...
# https://zhuanlan.zhihu.com/p/34023688
skopt scikit-learn optimission